...
public class Building {
	...
	public Building(int type, int rotation, int colorIndex, int gridX, int gridY, double tileSize) {
		...
		// buildins that are rotated 90 or 270 degrees appear 0.5 tile down. couldnt find source of the problem 
	...
	private void setImage() {
		...
		Image img = new Image(
				"/images/BuildingTile-Type" + type + "-Rotation" + rotation + "-Color" + colorIndex + ".png");
		...
		if (type == 2) {// type 2 is 1x1 building
			...
			if (rotation == 0 || rotation == 180) {
				...
				imageView.setFitWidth(3 * tileSize);
				imageView.setFitHeight(2 * tileSize);
			}
